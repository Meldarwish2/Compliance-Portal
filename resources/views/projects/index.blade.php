...
<div class="modal fade" id="deleteProjectModal{{ $project->id }}" tabindex="-1" aria-labelledby="deleteProjectModalLabel{{ $project->id }}" aria-hidden="true">
    ...
    <script>
    // Script to handle ZIP password visibility and option selection
    $(document).ready(function () {
    @foreach($projects as $project)
        const modalId = '#deleteProjectModal{{ $project->id }}';
        const passwordContainer = $('#passwordInputContainer{{ $project->id }}');
        const deleteOptionInput = $('#deleteOptionInput{{ $project->id }}');
        const zipPasswordInput = $('#zipPasswordInput{{ $project->id }}');

        $(modalId).find('.delete-option').on('change', function () {
            const selectedOption = $(this).val();
            deleteOptionInput.val(selectedOption);

            if (selectedOption === 'archive') {
                passwordContainer.show();
            } else {
                passwordContainer.hide();
                zipPasswordInput.val('');
            }
        });

        $(modalId).on('hidden.bs.modal', function () {
            $(modalId).find('input[type="radio"]').first().prop('checked', true);
            deleteOptionInput.val('delete');
            passwordContainer.hide();
            zipPasswordInput.val('');
        });

        $('#deleteProjectForm{{ $project->id }}').on('submit', function () {
            zipPasswordInput.val($('#zipPassword{{ $project->id }}').val());
        });
    @endforeach
});
